...
A `GET` request helps to download the projects report. Set `format` to `csv`, `json`,
or `xml` for a machine-readable file; the default is `xlsx`. The same `format`
values are supported for `module=components` and `module=licenses`.
